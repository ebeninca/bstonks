...
@bpDre.route('/dre/<companyId>')
def dre(companyId):
    current_app.logger.info("### DRE ###")

    respJson = json.loads(dreApi(companyId)[0])
    if(respJson[1] == 400):
        return render_template('dre.jinja', errorMsg=respJson[0])

    return render_template('dre.jinja', stocks=respJson, colnames=(respJson[0]).keys())
...
def dreApi(companyId):
    ...
    # = requests.args.get('companyId')
    resp = requests.get(
        "https://statusinvest.com.br/home/mainsearchquery?q=" + companyId)
    ...
    company = companyInfoJson[0]
    yearNow = datetime.datetime.now().year
    callUrl = "https://statusinvest.com.br/acao/getdre?"
    callUrl += "companyName=" + company['normalizedName']
